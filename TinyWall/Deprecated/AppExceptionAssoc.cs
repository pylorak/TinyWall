...
namespace PKSoft.Obsolete
{
    ...
    public sealed class AppExceptionAssoc
    {
        ...
        internal PKSoft.DatabaseClasses.SubjectIdentity ToNewFormat()
        {
            DatabaseClasses.SubjectIdentity ret = new DatabaseClasses.SubjectIdentity(ExceptionSubject.Construct(this.m_Executable, this.Service));
            if (0 != (Hashes?.Length ?? 0))
            {
                ret.AllowedSha1 = new List<string>();
                ret.AllowedSha1.AddRange(this.Hashes);
            }
            if (0 != (SearchPaths?.Length ?? 0))
            {
                ret.SearchPaths = new List<string>();
                ret.SearchPaths.AddRange(this.SearchPaths);
            }
            ret.Policy = this.ExceptionTemplate.ToNewFormat2().Policy;
        ...
        public string[] Hashes
        {
            get
            {
                if ((m_Hashes == null) && File.Exists(Executable))
                {
                    ...
                    {
                        m_Hashes = new string[] { TinyWall.Interface.Internal.Utils.HexEncode(hasher.ComputeHash(fs)) };
                    }
                }
        ...
        public bool SearchForFile(string pathHint = null)
        {
            ...
            string exec = RecursiveParser.ResolveString(this.Executable);
        ...
        public bool DoesExecutableSatisfy(AppExceptionAssoc exe)
        {
            ...
            else
            {
                ...
                if (string.Compare(RecursiveParser.ResolveString(this.Executable), RecursiveParser.ResolveString(exe.Executable), StringComparison.OrdinalIgnoreCase) != 0)
                    return false;
            }
        ...
        public bool IsSignatureValid
        {
            get
            {
                if (!m_IsSignatureValid.HasValue)
                    m_IsSignatureValid = TinyWall.Interface.Internal.WinTrust.VerifyFileAuthenticode(Executable);
