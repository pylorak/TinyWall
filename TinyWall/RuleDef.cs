...
namespace PKSoft
{
    ...
    public class RuleDef
    {
        ...
        public RuleDef(string name, PacketAction action, RuleDirection direction, Protocol protocol)
        {
            this.Name = name;
            this.Action = action;
        ...
        internal void ConstructRule(List<Rule> ruleset, string appid)
        {
            ...
            if (this.Protocol == WindowsFirewall.Protocol.TcpUdp)
            {
                ...
                pProt.ConstructRule(ruleset, appid + " TCP");

                // For UDP
                pProt = Utils.DeepClone(this);
                pProt.Protocol = WindowsFirewall.Protocol.UDP;
                pProt.ConstructRule(ruleset, appid + " UDP");
            }
            else if (this.Protocol == WindowsFirewall.Protocol.ICMP)
            {
                ...
                pProt.ConstructRule(ruleset, appid + " ICMPv4");

                // For ICMPv6
                pProt = Utils.DeepClone(this);
                pProt.Protocol = WindowsFirewall.Protocol.ICMPv6;
                pProt.ConstructRule(ruleset, appid + " ICMPv6");
            }
            else if (this.Direction == WindowsFirewall.RuleDirection.InOut)
            {
                ...
                pDir.ConstructRule(ruleset, appid + " in");

                // For OUT
                pDir = Utils.DeepClone(this);
                pDir.Direction = WindowsFirewall.RuleDirection.Out;
                pDir.ConstructRule(ruleset, appid + " out");
            }
            else
            {
                Rule r = new Rule(appid + " " + this.Name, string.Empty, ProfileType.All, this.Direction, this.Action, this.Protocol);
